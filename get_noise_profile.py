# Justin 2019
# Code is from the python notebooks. Tihs is a modified version of silence_cutter.py
# for outputing the best noise profile based on length and lack of voice.
...
if __name__ == "__main__":
    ...
    for file in os.listdir(directory):
        ...
        if filename.endswith(".wav"):
            ...
            # Compute the longest silence:
            i = 1
            best_silence = [0,1]
            best_length = 0
            while i < len(sounds):
                # check silence between [i][0] and [i-1][1]
                length = sounds[i][0] - sounds[i-1][1]
                if length > best_length:
                    best_length = length
                    best_silence = [sounds[i-1][1], sounds[i][0]]
                i += 1
            ...
            output_path = args.output_dir + filename + ".noise.wav"
            print("outputting to :", output_path, sr)
            if len(t) <= int(best_silence[1]*sr):
                print(filename, " HAS A PROBLEM, algorithm failed to detect silences.")
                exit()
            else:
                output = t[int(best_silence[0]*sr):int(best_silence[1]*sr)]
